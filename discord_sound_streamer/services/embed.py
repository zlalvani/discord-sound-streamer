...
def _apply_track_list_to_embed(embed: Embed, tracks: List[Track]) -> None:
    for ordinal, track in enumerate(tracks, start=1):
        ...
        embed.add_field(name=f'{ordinal}. {track.title}', value=_build_track_link(track), inline=True)
...
def build_track_embed(track: Track, title: str = 'Now Playing', show_time_remaining: bool = False) -> Embed:
    embed = Embed(title=title, color=0x000000)
    embed.add_field(name='Title', value=track.title, inline=True)
    embed.add_field(name='Author', value=track.author, inline=True)
    embed.add_field(name='Remaining' if show_time_remaining else 'Length', value=str(timedelta(milliseconds=track.length - (1000 * track.position if show_time_remaining else 0))).split('.')[0], inline=True)
